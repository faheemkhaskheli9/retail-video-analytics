"""A minimal IoU-based multi-object tracker.

This is a simplified SORT-style tracker: no Kalman filter, just greedy
IoU matching between the previous frame's tracks and the current frame's
detections, with track aging so brief missed detections do not immediately
kill a track. It is intentionally dependency-light (numpy only) so the MVP
runs anywhere the detector backend runs.
"""
...
class IOUTracker:
    ...
    def update(self, detections: list[Detection]) -> list[Track]:
        ...
        # Greedy matching: repeatedly pick the highest-IoU (track, detection)
        # pair above threshold until no more pairs qualify. O(n*m) is plenty
        # for the object counts a single retail-camera frame realistically has.
        ...
        # Age (and possibly drop) unmatched tracks.
